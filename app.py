...
def model():
    ...
    if request.method=='POST':
        ...
        if models==1:
            ...
            print(eta)
            msg = 'Accuracy  for ExtraTreesClassifier is ' + str(eta)

        elif models ==9:
            print("===============")
            # Define base classifiers
            base_classifiers = [ ('rf', RandomForestClassifier(n_estimators=100, random_state=42)),
            ('svm', SVC(kernel='linear', probability=True, random_state=42)) ]
            # Define the meta classifier
            meta_classifier = LogisticRegression(max_iter=1000, random_state=42)
            # Create stacking classifier
            stc = StackingClassifier(estimators=base_classifiers,
            final_estimator=meta_classifier )

            stc.fit(X_train[:100], y_train[:100])
            stp = stc.predict(X_test[:100])
            stc = accuracy_score(y_test[:100], stp)
            print(sca)
            msg = 'Accuracy  for StackingClassifier is ' + str(sca)
...
def prediction():
    ...
    if  request.method == 'POST':
        print('2222')
        IsBeta = float(request.form['IsBeta'])

        RtpStateBitfield = float(request.form['RtpStateBitfield'])

        IsSxsPassiveMode = float(request.form['IsSxsPassiveMode'])

        DefaultBrowsersIdentifier = float(request.form['DefaultBrowsersIdentifier'])

        AVProductStatesIdentifier = float(request.form['AVProductStatesIdentifier'])

        AVProductsInstalled = float(request.form['AVProductsInstalled'])

        HasTpm = float(request.form['HasTpm'])

        CountryIdentifier = float(request.form['CountryIdentifier'])

        CityIdentifier = float(request.form['CityIdentifier'])

        OrganizationIdentifier = float(request.form['OrganizationIdentifier'])

        GeoNameIdentifier = float(request.form['GeoNameIdentifier'])

        LocaleEnglishNameIdentifier = float(request.form['LocaleEnglishNameIdentifier'])

        OsBuild = float(request.form['OsBuild'])

        OsSuite = float(request.form['OsSuite'])

        IsProtected = float(request.form['IsProtected'])

        SMode = float(request.form['SMode'])

        Firewall = float(request.form['Firewall'])

        Census_InternalPrimaryDiagonalDisplaySizeInInches = float(request.form['Census_InternalPrimaryDiagonalDisplaySizeInInches'])

        Census_InternalPrimaryDisplayResolutionHorizontal = float(request.form['Census_InternalPrimaryDisplayResolutionHorizontal'])

        Census_InternalPrimaryDisplayResolutionVertical = float(request.form['Census_InternalPrimaryDisplayResolutionVertical'])

        Census_OSBuildNumber = float(request.form['Census_OSBuildNumber'])

        Census_OSBuildRevision = float(request.form['Census_OSBuildRevision'])

        Census_OSInstallLanguageIdentifier = float(request.form['Census_OSInstallLanguageIdentifier'])

        Wdft_IsGamer = float(request.form['Wdft_IsGamer'])

        Wdft_RegionIdentifier = float(request.form['Wdft_RegionIdentifier'])
